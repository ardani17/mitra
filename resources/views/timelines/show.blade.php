...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Detail Progres Timeline</h1>
        <div class="flex space-x-2">
            <a href="{{ route('timelines.edit', $timeline) }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                Edit Progres
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        <div class="flex justify-between items-start mb-6">
            <div>
                <h2 class="text-2xl font-bold text-gray-800">{{ $timeline->milestone }}</h2>
                <p class="text-gray-600 mt-2">Project: {{ $timeline->project->name }}</p>
                <p class="text-gray-600">Company: {{ $timeline->project->user->company_name ?? 'N/A' }}</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                <h3 class="text-lg font-semibold text-gray-800 mb-4">Informasi Progres</h3>
                <div class="space-y-3">
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Description</label>
                        <div class="mt-1 text-sm text-gray-900">{{ $timeline->description ?? '-' }}</div>
                    </div>
                    <div>
            ...
            <div>
                <h3 class="text-lg font-semibold text-gray-800 mb-4">Timeline Dates</h3>
                <div class="space-y-3">
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Planned Date</label>
                        <div class="mt-1 text-sm text-gray-900">{{ $timeline->planned_date->format('d M Y') }}</div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Actual Date</label>
                        <div class="mt-1 text-sm text-gray-900">{{ $timeline->actual_date ? $timeline->actual_date->format('d M Y') : '-' }}</div>
                    </div>
                    @if($timeline->actual_date && $timeline->planned_date)
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Days Difference</label>
            ...
            <div>
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <div class="mt-1 text-sm text-gray-900">{{ $timeline->project->name }}</div>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Total Progres</label>
                        <div class="mt-1 text-sm text-gray-900">{{ $timeline->project->timelines()->count() }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 pt-6 border-t border-gray-200">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <span class="text-sm font-medium text-gray-900">{{ $timeline->created_at->format('d M Y H:i') }}</span>
                </div>
                <div>
                    <span class="text-sm text-gray-600">Last Updated:</span>
                    <span class="text-sm font-medium text-gray-900">{{ $timeline->updated_at->format('d M Y H:i') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <form action="{{ route('timelines.update-status', $timeline) }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            @csrf
            <div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <h2 class="text-xl font-bold text-gray-800 mb-4">Related Activities</h2>
        <div class="space-y-4">
            ...
            <div class="border border-gray-200 rounded-lg p-4">
                <div class="flex justify-between items-start">
                    <div>
                        <h3 class="text-sm font-medium text-gray-900">{{ $activity->description }}</h3>
                        <p class="text-sm text-gray-500 mt-1">By {{ $activity->user->name }}</p>
                    </div>
            ...
            @empty
            <p class="text-gray-500">Tidak ada aktivitas ditemukan untuk progres ini.</p>
            @endforelse
        </div>
    </div>
</div>
