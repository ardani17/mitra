...
<div class="container mx-auto px-4 py-6 sm:py-8">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 space-y-4 sm:space-y-0">
        <div class="min-w-0 flex-1">
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Edit Transaksi Cashflow</h1>
            <p class="text-gray-600 mt-1 text-sm sm:text-base">ID: #{{ $cashflow->id }}</p>
        </div>
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-xs sm:text-sm text-yellow-700">
                        <p class="hidden sm:block">Transaksi ini dibuat otomatis dari sistem. Perubahan yang Anda lakukan mungkin akan ditimpa jika status referensi berubah.</p>
                        <p class="sm:hidden">Transaksi ini dibuat otomatis dari sistem.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
        <form method="POST" action="{{ route('finance.cashflow.update', $cashflow) }}">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="amount" id="amount" 
                           value="{{ old('amount', number_format($cashflow->amount, 0, ',', '.')) }}" required
            ...
            <div class="mt-4 sm:mt-6">
                ...
                <textarea name="description" id="description" rows="4" required
                          ...
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base @error('description') border-red-500 @enderror">{{ old('description', $cashflow->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-4 sm:mt-6">
                ...
                <textarea name="notes" id="notes" rows="3"
                          ...
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base @error('notes') border-red-500 @enderror">{{ old('notes', $cashflow->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-4 sm:mt-6 bg-gray-50 border border-gray-200 rounded-lg p-4">
                <div class="flex">
                    <div class="flex-shrink-0">
                    ...
                    <div class="ml-3">
                        <h3 class="text-sm font-medium text-gray-800">Perbandingan Perubahan</h3>
                        <div class="mt-2 text-sm text-gray-700">
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                <div>
                                    <h5 class="text-xs font-medium text-gray-600 mb-2">Data Saat Ini</h5>
                                ...
                                <div>
                                    <h5 class="text-xs font-medium text-gray-600 mb-2">Preview Perubahan</h5>
