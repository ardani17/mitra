...
<div class="container mx-auto px-4 py-6 sm:py-8">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 space-y-4 sm:space-y-0">
        <h1 class="text-2xl sm:text-3xl font-bold text-slate-800">Analisis Profitabilitas Proyek</h1>
        <div class="flex space-x-2">
            <a href="{{ route('reports.index') }}"
               class="btn-secondary text-sm sm:text-base py-2 px-3 sm:px-4 text-center">
                Kembali ke Laporan
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
        ...
        <div class="card p-3 sm:p-6">
            <div class="flex items-center">
                ...
                <div class="ml-2 sm:ml-4 min-w-0 flex-1">
                    ...
                    <p class="text-lg sm:text-2xl font-semibold text-gray-900">{{ $totalProjects }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="card p-3 sm:p-6">
            <div class="flex items-center">
                ...
                <div class="ml-2 sm:ml-4 min-w-0 flex-1">
                    <p class="text-xs sm:text-sm font-medium text-gray-600 truncate">Proyek Profit</p>
                    <p class="text-lg sm:text-2xl font-semibold text-green-600">{{ $profitableProjects }}</p>
                    <p class="text-xs sm:text-sm text-gray-500">{{ $totalProjects > 0 ? number_format(($profitableProjects / $totalProjects) * 100, 1) : 0 }}%</p>
                </div>
            </div>
        </div>
        ...
        <div class="card p-3 sm:p-6">
            <div class="flex items-center">
                ...
                <div class="ml-2 sm:ml-4 min-w-0 flex-1">
                    <p class="text-xs sm:text-sm font-medium text-gray-600 truncate">Proyek Rugi</p>
                    <p class="text-lg sm:text-2xl font-semibold text-red-600">{{ $lossProjects }}</p>
                    <p class="text-xs sm:text-sm text-gray-500">{{ $totalProjects > 0 ? number_format(($lossProjects / $totalProjects) * 100, 1) : 0 }}%</p>
                </div>
            </div>
        </div>
        ...
        <div class="card p-3 sm:p-6">
            <div class="flex items-center">
                <div class="p-2 sm:p-3 rounded-full {{ $avgProfitMargin >= 0 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }}">
                    <svg class="w-4 h-4 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                    </svg>
                </div>
                <div class="ml-2 sm:ml-4 min-w-0 flex-1">
                    <p class="text-xs sm:text-sm font-medium text-gray-600 truncate">Rata-rata Margin</p>
                    <p class="text-lg sm:text-2xl font-semibold {{ $avgProfitMargin >= 0 ? 'text-green-600' : 'text-red-600' }}">
    ...
    <div class="card p-4 sm:p-6">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 sm:mb-4 space-y-2 sm:space-y-0">
            <h3 class="text-base sm:text-lg font-semibold text-slate-800">Detail Profitabilitas Proyek</h3>
            ...
            <div class="border-b border-gray-200 p-4 hover:bg-gray-50 transition-colors duration-200">
                ...
                <div class="flex justify-between items-start mb-3">
                    ...
                    <div class="ml-3 flex flex-col items-end space-y-1">
                        ...
                        @endphp
                        <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $performanceClass }}">
                            {{ $performance }}
                        </span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 mb-3">
                    <div>
                        <label class="text-xs font-medium text-gray-500">Nilai Plan</label>
                        <div class="text-sm font-semibold text-gray-900 mt-1">
                            Rp {{ number_format($project->planned_total_value / 1000000, 1) }}M
                        </div>
                    </div>
                    <div>
                ...
                <div class="flex items-center justify-between">
                    <span class="text-xs font-medium text-gray-500">Profit Margin</span>
                    <div class="flex items-center space-x-2">
                        <span class="text-sm font-semibold {{ $project->profit_margin >= 0 ? 'text-green-600' : 'text-red-600' }}">
                            {{ number_format($project->profit_margin, 2) }}%
                        </span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mt-6 sm:mt-8">
        <!-- Top Performers -->
        <div class="card p-4 sm:p-6">
            <h3 class="text-base sm:text-lg font-semibold text-slate-800 mb-3 sm:mb-4">Top Performers (Margin > 20%)</h3>
            <div class="space-y-2 sm:space-y-3">
                @php
                    $topPerformers = $projects->where('profit_margin', '>', 20)->take(5);
                @endphp
                @forelse($topPerformers as $project)
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 bg-green-50 rounded-lg space-y-2 sm:space-y-0">
                    <div class="min-w-0 flex-1">
                        <div class="text-sm font-medium text-gray-900 truncate" title="{{ $project->name }}">{{ Str::limit($project->name, 25) }}</div>
                    ...
                    <div class="text-right">
                        ...
                        <div class="text-xs text-gray-500">Rp {{ number_format($project->net_profit / 1000000, 1) }}M</div>
                    </div>
                </div>
                @empty
                <div class="text-sm text-gray-500 text-center py-3 sm:py-4">
        ...
        <!-- Poor Performers -->
        <div class="card p-4 sm:p-6">
            <h3 class="text-base sm:text-lg font-semibold text-slate-800 mb-3 sm:mb-4">Perlu Perhatian (Margin < 0%)</h3>
            <div class="space-y-2 sm:space-y-3">
                @php
                    $poorPerformers = $projects->where('profit_margin', '<', 0)->take(5);
                @endphp
                @forelse($poorPerformers as $project)
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 bg-red-50 rounded-lg space-y-2 sm:space-y-0">
                    <div class="min-w-0 flex-1">
                        <div class="text-sm font-medium text-gray-900 truncate" title="{{ $project->name }}">{{ Str::limit($project->name, 25) }}</div>
                        <div class="text-xs text-gray-500">{{ $project->code }}</div>
                    </div>
    ...
    <div class="card p-4 sm:p-6 mt-6 sm:mt-8">
        <h3 class="text-base sm:text-lg font-semibold text-slate-800 mb-3 sm:mb-4">Rekomendasi Perbaikan</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
            <div class="bg-blue-50 p-3 sm:p-4 rounded-lg">
                <h4 class="font-semibold text-blue-800 mb-2 text-sm sm:text-base">Optimasi Biaya</h4>
            ...
            <div class="bg-green-50 p-3 sm:p-4 rounded-lg">
                <h4 class="font-semibold text-green-800 mb-2 text-sm sm:text-base">Replikasi Sukses</h4>
            ...
            <div class="bg-yellow-50 p-3 sm:p-4 rounded-lg">
                <h4 class="font-semibold text-yellow-800 mb-2 text-sm sm:text-base">Pricing Strategy</h4>
