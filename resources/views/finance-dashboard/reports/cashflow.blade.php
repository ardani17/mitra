...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 space-y-4 sm:space-y-0">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-slate-800">Laporan Cash Flow</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
        ...
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg shadow-sm p-4 sm:p-6 text-white">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <p class="text-blue-100 text-xs sm:text-sm font-medium">Arus Kas Bersih</p>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4 sm:p-6 mb-6 sm:mb-8">
        <h2 class="text-lg sm:text-xl font-semibold text-slate-800 mb-4 sm:mb-6">Laporan Arus Kas</h2>
                ...
                <div class="card p-4 border border-slate-200 rounded-lg">
                    <div class="flex items-start justify-between mb-3">
                        <div class="flex-1">
                            <div class="flex items-center space-x-2 mb-2">
                                <span class="text-xs text-slate-600">{{ $transaction->transaction_date->format('d/m/Y') }}</span>
                            ...
                            <h4 class="font-medium text-slate-800 text-sm break-words">{{ Str::limit($transaction->description, 40) }}</h4>
                            @if($transaction->notes)
                                <p class="text-xs text-slate-500 mt-1">{{ Str::limit($transaction->notes, 50) }}</p>
                            @endif
                        </div>
                        <div class="text-right ml-3">
                            <div class="font-semibold {{ $transaction->type === 'income' ? 'text-green-600' : 'text-red-600' }} text-sm break-words">
                    ...
                    <div class="grid grid-cols-2 gap-3 text-xs border-t border-slate-100 pt-3">
                        <div>
                            ...
                            <div class="font-medium text-slate-800">{{ $transaction->category->name }}</div>
                        </div>
                        <div>
                            <span class="text-slate-600">Proyek:</span>
                            <div class="font-medium text-slate-800">{{ $transaction->project?->name ?? '-' }}</div>
                        </div>
                        <div class="col-span-2">
                            <span class="text-slate-600">Saldo Berjalan:</span>
                            <div class="font-semibold {{ $transaction->running_balance >= 0 ? 'text-green-600' : 'text-red-600' }} break-words">
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-slate-200">
                        <th class="text-left py-3 px-4 font-semibold text-slate-700 text-sm">Tanggal</th>
                        <th class="text-left py-3 px-4 font-semibold text-slate-700 text-sm">Deskripsi</th>
                        <th class="text-left py-3 px-4 font-semibold text-slate-700 text-sm">Kategori</th>
                        <th class="text-left py-3 px-4 font-semibold text-slate-700 text-sm">Proyek</th>
                        <th class="text-center py-3 px-4 font-semibold text-slate-700 text-sm">Tipe</th>
                        <th class="text-right py-3 px-4 font-semibold text-slate-700 text-sm">Jumlah</th>
                        <th class="text-right py-3 px-4 font-semibold text-slate-700 text-sm">Saldo Berjalan</th>
                        <th class="text-center py-3 px-4 font-semibold text-slate-700 text-sm">Status</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($transactions as $transaction)
                        <tr class="border-b border-slate-100 hover:bg-slate-50">
                            ...
                            <td class="py-3 px-4">
                                <div class="font-medium text-slate-800 text-sm">{{ Str::limit($transaction->description, 40) }}</div>
                                @if($transaction->notes)
                            ...
                            <td class="py-3 px-4 text-right font-medium {{ $transaction->type === 'income' ? 'text-green-600' : 'text-red-600' }} text-sm">
                                {{ $transaction->type === 'income' ? '+' : '-' }}{{ $transaction->formatted_amount }}
                            </td>
                            <td class="py-3 px-4 text-right font-semibold {{ $transaction->running_balance >= 0 ? 'text-green-600' : 'text-red-600' }} text-sm">
                                Rp {{ number_format($transaction->running_balance, 0, ',', '.') }}
                            </td>
                            <td class="py-3 px-4 text-center">
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $transaction->status === 'confirmed' ? 'bg-green-100 text-green-800' : ($transaction->status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
    ...
    @if($transactions->count() > 0)
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4 sm:p-6">
                <h3 class="text-base sm:text-lg font-semibold text-slate-800 mb-4">Analisis Pemasukan</h3>
                <div class="space-y-3">
                    @foreach($transactions->where('type', 'income')->groupBy('category.name') as $categoryName => $categoryTransactions)
                        <div class="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                            <div class="flex-1 mr-3">
                                <p class="font-medium text-slate-800 text-sm break-words">{{ $categoryName }}</p>
                                <p class="text-xs sm:text-sm text-slate-600">{{ $categoryTransactions->count() }} transaksi</p>
                            </div>
                            <div class="text-right flex-shrink-0">
                                <p class="font-semibold text-green-600 text-sm break-words">
            ...
            <!-- Expense Analysis -->
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4 sm:p-6">
                <h3 class="text-base sm:text-lg font-semibold text-slate-800 mb-4">Analisis Pengeluaran</h3>
                <div class="space-y-3">
                    @foreach($transactions->where('type', 'expense')->groupBy('category.name') as $categoryName => $categoryTransactions)
                        <div class="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                            ...
                            <div class="text-right flex-shrink-0">
                                <p class="font-semibold text-red-600 text-sm break-words">
                                    Rp {{ number_format($categoryTransactions->sum('amount'), 0, ',', '.') }}
                                </p>
                                <p class="text-xs text-slate-500">
