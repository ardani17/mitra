export default {
  ...
  methods: {
    async loadRootFolders() {
      ...
      try {
        const response = await fetch('/telegram-bot/folder-tree-lazy?' + new URLSearchParams({
          parent: '',
    ...
    async handleNodeExpand(folderPath) {
      ...
      if (this.expandedNodes.has(folderPath)) {
        ...
        this.expandedNodes = new Set(this.expandedNodes); // Force reactivity
        return;
      }
      ...
      this.loadingNodes = new Set(this.loadingNodes); // Force reactivity
      ...
      try {
        ...
        if (data.success) {
          ...
          if (folder) {
            Vue.set(folder, 'children', data.folders);
          }
